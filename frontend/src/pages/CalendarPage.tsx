 ...
 const [events, setEvents] = useState<CalendarEvent[]>([
    {
      ...
      end: new Date(2025, 5, 13),  // 2025년 6월 12일
      color: 'red',
      category: '기획',
    },
    {
      ...
      end: new Date(2025, 5, 16), // 2025년 6월 15일
      color: 'blue',
      category: '디자인',
    },
  ]);
  ...
  const colorMatch = color === '' || e.color === color;
      ...
      <div className="flex items-center justify-between mb-4">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent>
              {months.map((m) => (
                <DropdownMenuItem
                  key={m}
                  onClick={() => setDate(new Date(date.getFullYear(), m - 1, 1))}
                >
                  {m}월
                </DropdownMenuItem>
              ))}
        ...
        <div className="flex gap-2 flex-1 justify-end">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent>
              {/* 전체보기 */}
              <DropdownMenuItem onClick={() => setColor('')}>
                {color === '' && <span>✔ </span>} 전체보기
              </DropdownMenuItem>
              {colors.map((col) => (
                <DropdownMenuItem key={col} onClick={() => setColor(col)}>
                  {color === col && <span>✔ </span>}
                  {col}
                </DropdownMenuItem>
              ))}
